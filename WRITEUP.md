...
However distortion from camera input due to change in focal length and/or image size will affect the model because the model may fail to make sense of the distorted input thereby reducing the model detection accuracy. One major approach to resolve this issue is by correcting the images while training the model and specifying a threshold skews over carefully selected ranges.

## Answer to reviewer's feedback

### Discussing the effect of lighting to end user requirements

Object detection is now widely used in self-driving cars as an important software system to detect objects such as pedectrian, automobiles, traffic sign boards etc. In order to ensure the accuracy of these objects detection, the vehicles should have eyes which are not the real eyes but cameras. Detection of the objects is considered very important since it includes saftey of the public. Thefore the ability of these object detection application such the one demonstrated in this project to detect accurately the precise object will depend on such factors as high-resolution camera with effective focal-length, high definition lighting effect for spotlight detection which would allow the AI application to detect the object accurately as demonstrated by this [study](https://www.researchgate.net/publication/337527327_Traffic_Light_Detection_Using_Tensorflow_Object_Detection_Framework).  During poor lighting, model detection accuracy may fail and can drop dramatically to zero. However this effect can be mitigated by using good hardware than can process images from poorly lit regions before passing it to the model. 
